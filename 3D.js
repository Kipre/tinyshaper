...
//   const maxi = (accumulator, currentValue) => [Math.max(accumulator[0], currentValue[0]), Math.max(accumulator[1], currentValue[1])];
//   const [maxX, maxY] = curvePoints.reduce(maxi);
//   for (var pointIndex=0; pointIndex<curvePoints.length;pointIndex++) {
//     var [x, y] = curvePoints[pointIndex]
...
//     const arrays = lathePoints(points, data.startAngle, data.endAngle, data.divisions, data.capStart, data.capEnd);
  ...
  // rotates around Y axis.
  function splitPoints(points, numDivisions=20, step=1) { 
    const positions = [];
    const texcoords = [];
    const indices = [];
    ...
    for (let division = 0; division <= numDivisions; ++division) {
      const u = division / numDivisions;
      points.forEach((p, ndx) => {
        positions.push(p[0], p[1], division*step - numDivisions*step/2);
        // note: this V is wrong. It's spacing by ndx instead of distance along curve
