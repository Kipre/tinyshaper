...
class Redactor {
    ...
    initialize() {
        ...
        this.z = new Canvas(this.board, 'z', this.container,
            ...
            'board-canvas',
            (x, y, continuity) => { this.assignCurrentPoint(x, y, continuity); });
    ...
    assignCurrentPoint(x, y, continuity) {
        this.showPointInfo(x, y, continuity);
    }
...
class Canvas {
    ...
    constructor(board, profile, parent, scales, className = null, pointCallback = null) {
    ...
    getPoints(profile) {
        ...
        pointsArray = pointsArray.map((p, i) => {
            ...
            if (j[4] >= 0) {
                let newPoint = new ParentPoint(...j);
                newPoint.continuity = this.board.continuity[j[4]] // number is in 5th position
                return newPoint
            } else {
        ...
        for (var i = 0; i < pointsArray.length; i++) { // double loop is bad
    ...
    setCurrentPoint (point) {
        ...
        this.pointCallback(...this.from([point.x, point.y]), point.continuity);
    }
    ...
    onDown(e) {
        ...
        if (!this.isDragging) {
            this.setCurrentPoint(new Point(0, 0, 0, 0, 0));
        }
        this.startX = mx;
        this.startY = my;
        this.render();
...
class App {
    ...
    constructor(board) {
        ...
        this.toolbar = ReactDOM.render(React.createElement(Toolbar, {
            redactorView: true,
            initializeRedactor: () => { this.redactor.initialize(); },
            initializeThreeD: () => { this.threeDView.initialize(); },
            pointUpdateCallback: (x, y) => { this.redactor.updateCurrentPoint(x, y) }
        }, null), this.toolbarDOM);

        this.redactor.showPointInfo = (x, y) => {this.toolbar.showPointInfo(x, y);}
    }
}
...
class Toolbar extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = { redactorView: props.redactorView,
            x: 0,
            y: 0,
            continuity: false
        }
    ...
    handleViewChange() {
        const newState = !this.state.redactorView;
        if (newState) {
            this.props.initializeRedactor();
        } else {
            this.props.initializeThreeD();
        }
        this.setState({ ...this.state, redactorView: newState })
    }
    ...
    showPointInfo(x, y, continuity) {
        this.setState({...this.state, x: x, y: y, continuity: continuity})
    }

    render() {
        return React.createElement("div", null, React.createElement("button", {
            onClick: () => { this.handleViewChange(); }
        }, "view mode"), React.createElement("form", {
                onSubmit: this.handlePointSubmit
            }, React.createElement("label", {htmlFor: 'pointx'}, "Y :"), React.createElement("input", {
                type: "text",
                value: this.state.x,
                onChange: this.handlePointChange,
                id: 'pointx'
            }), React.createElement("br", null), React.createElement("label", {htmlFor: 'pointy'}, "X :"),
            React.createElement("input", {
                type: "text",
                value: this.state.y,
                onChange: this.handlePointChange,
                id: 'pointy'
            }), React.createElement("br", null), React.createElement("label", null, "Continuity :"), 
            React.createElement("input", {
                type: "checkbox",
                checked: false,
